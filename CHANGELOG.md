...
- **Quick Start — PANIC RESET** button next to *PANIC / BLACKOUT*. It is a Script that stops every generated function, then starts a *Reset: neutral state* scene (shutter open, no effects, Pan/Tilt centred, intensity 0). A plain scene is not enough: intensity and colour are HTP in QLC+, so a scene at 0 can't pull down a look that is still running. Clears Doctor D008.
