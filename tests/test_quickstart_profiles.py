...
def test_20minutes_legend_matches_liquidbar():
...
def test_extract_style_liquidbar():
    st = extract_style_file(os.path.join(CORPUS, "LiquidBar_v14.qxw"))
    assert (st.page_w, st.page_h) == (1650, 884)
    assert (st.btn_w, st.btn_h) == (164, 42)
    assert st.gap == 8
    assert st.font_button.startswith("Roboto,10")
    assert st.source == "LiquidBar_v14.qxw"


def test_builtin_liquidbar_equals_extracted():
    built = load_style("liquidbar").to_dict()
    ext = extract_style_file(os.path.join(CORPUS, "LiquidBar_v14.qxw")).to_dict()
...
def test_style_defaults_and_list():
    ...
    assert {s["id"] for s in list_styles()} >= {"default", "liquidbar"}
...
def test_options_roundtrip(client):
    ...
    d = client.post("/api/quickstart/options",
                    json={"nomenclature": "20minutes", "style": "liquidbar"}).get_json()
    assert d["nomenclature"] == "20minutes" and d["style_data"]["btn_w"] == 164
    d = client.post("/api/quickstart/options",
                    json={"style_path": os.path.join(CORPUS, "SangAKlang_v41.qxw")}).get_json()
    assert d["style"] == "custom" and d["style_data"]["source"] == "SangAKlang_v41.qxw"
...
def test_options_style_upload(client):
    with open(os.path.join(CORPUS, "LiquidBar_v14.qxw"), "rb") as fh:
        data = {"style_file": (fh, "LiquidBar_v14.qxw")}
