...
- Porter step 2 (after Giovanni's test): ticking a VC page/frame ticks everything inside (partly ticked frames show a dash); the VC list has its own ✓ All / ✗ None; dependencies resolve automatically as you tick (no *Resolve Dependencies* button — **Next** always works when something is ticked); Porter messages use the app's single status bar, so a stale "No workspace is open" from another action no longer sits under the Porter.
