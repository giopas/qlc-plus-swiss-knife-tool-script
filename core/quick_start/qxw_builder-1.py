...
def _compute_orientation(y_mm: int, stage_h_mm: int,
                         ...
                         custom_z_rot=None):
    """
    Compute fixture orientation (XRot, YRot, ZRot) for QLC+ 3D monitor.

    If custom rotations are provided (not None), they are used directly.
    Otherwise, orientation is derived from the fixture's height:
      - Floor  (y < 15% of stage height): point UP   → XRot=315 (-45°)
      - Top    (y > 65% of stage height): point DOWN  → XRot=65
      - Mid:                               horizontal → XRot=0
    """
    if custom_x_rot is not None:
        ...
        if ratio < 0.15:
            x_rot = 315   # point upward (~45° above horizontal)
        elif ratio > 0.65:
            x_rot = 65    # point downward (~65° below horizontal)
        else:
            x_rot = 0     # horizontal
...
def build_qxw(rig: list,
    ...
    rig : list[dict]
        ...
        and optionally x_rot, y_rot, z_rot for custom orientation.
    functions : list[ET.Element]
    ...
    # QLC+ 3D Monitor uses a CENTERED coordinate system:
    #   X: 0 = center stage,  -W/2 = stage right, +W/2 = stage left
    #   Y: 0 = floor,  positive = up
    #   Z: 0 = center depth,  -D/2 = back,  +D/2 = front
    # Our placement tool uses corner-origin (0,0 = back-right corner):
    #   x_mm: 0 → stage_w_mm  (right to left)
    #   z_mm: 0 → stage_d_mm  (back to front)
    #   y_mm: 0 → stage_h_mm  (floor to ceiling)
