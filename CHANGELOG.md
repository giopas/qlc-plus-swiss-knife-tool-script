...
- **Quick Start — one button at a time**: all looks, effects and group buttons sit in one *SHOW* solo frame, so pressing a button switches the previous one off (looks and effects no longer mix). The page fills the VC style's page width (1650 × 884 by default); the preview in step 4 fits the window and shows the button colours.
- **Quick Start — fixture definitions saved next to the workspace** (`<Manufacturer>-<Model>.qxf`, the name QLC+ looks for). Without them QLC+ loads unknown fixtures as plain dimmers (no colour, RGB effects dark). API `POST /api/quickstart/save-qxf`.
