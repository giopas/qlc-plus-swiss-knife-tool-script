...
function _renderSongList() {
  ...
  list.innerHTML = _songRows.map((row, i) => {
    ...
    // Fallback: if we still have no parent, try matching by the stored assignment
    // name (qxw_name) — this finds the original function even when the clone was
    // generated by an older version without SwissKnifeClone or (Setlist) suffix
    if (!parent && hasAssign && row.qxw_name) {
      const byName = _functions.find(f => f.name === row.qxw_name && f.id !== row.qxw_id);
      if (byName) parent = byName;
    }
    if (parent) {
