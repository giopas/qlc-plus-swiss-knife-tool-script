...
- Corpus: `QuickStart_6fix.qxw`, a Quick Start output (2 truss spots + 4 floor PARs) generated by `tools/make_quickstart_sample.py` through the real routes, plus a golden-file test proving Quick Start output is byte-identical run to run. Doctor finds 0 errors and one warning on it (D008: no real PANIC RESET yet).
